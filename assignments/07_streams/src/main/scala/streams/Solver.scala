...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream()
      ...
      val nextPossibleMoves = newNeighborsOnly(neighborsWithHistory(block, moves), explored)
      nextPossibleMoves #::: from( initial.tail #::: nextPossibleMoves, explored + block)
    }
  }
  ...
  lazy val solution: List[Move] = {
    def shorter[T](a: List[T], b: List[T]) = if (a.length < b.length)a else b

    val paths = pathsToGoal
    if (paths.isEmpty) List()
    else (pathsToGoal foldLeft (pathsToGoal.head._2))((previous, next)=> shorter(previous, next._2))
  }
}
